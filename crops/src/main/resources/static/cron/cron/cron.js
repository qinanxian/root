...
$(function() {
  var defaultValue = window.defaultValue;
	...
	$("#cron").val(defaultValue.expr);
	$("#runTimeMessage").html("最近" + defaultValue.times + "次运行时间:");
	...
	cron.change(function () {
	    ...
	    //设置最近五次运行时间
			// 获取token
      var token = get_param('X-SESSION-TOKEN');
      var baseUrl = window.parent.baseUrl;
      var onChange = window.onChange;
      var value = $("#cron").val();
      onChange && onChange(value, defaultValue.times);
	    $.ajax({
	        type: 'post',
	        url: baseUrl + '/batch/crontab/predictor/',
          data: {
	          expr: value,
          ...
          beforeSend: function (xhr) {
	        ...
	        success: function (data) {
	            if (data && data.length == defaultValue.times) {
	                var strHTML = "<ul>";
