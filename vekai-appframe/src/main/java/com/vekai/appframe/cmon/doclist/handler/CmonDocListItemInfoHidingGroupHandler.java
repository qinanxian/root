...
@Component
public class CmonDocListItemInfoHidingGroupHandler extends MapDataOneHandler {
    ...
    public int save(DataForm dataForm, MapObject object) {
        ...
        String groupId = object.getValue("groupId").strValue();
        if (StringKit.isBlank(groupId)) {
            String groupCode = object.getValue("groupCode").strValue();
            if(!StringKit.isBlank(groupCode)){
                String sql = "select * from CMON_DOCLIST_GROUP where DOCLIST_ID=:doclistId and GROUP_CODE=:groupCode";
                MapObject groupObject = mapObjectCruder.getMapObjectQuery().selectOne(sql,
                        MapKit.mapOf("doclistId", doclistId, "groupCode", groupCode));
                dbMapdata.put("groupId", groupObject.getValue("groupId").strValue());
            }
        }

        MapObject dbPkMapdata = getDbPkMapData(dataForm, object);
        togglePrimaryKey(dataForm, dbMapdata);
        dbPkMapdata.put("itemId",dbMapdata.getValue("itemId").toString());
