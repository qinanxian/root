...
var ProcessDiagramGenerator = {
	...
	initProcessDiagramCanvas: function (processDefinitionDiagramLayout) {
		...
		var activities = processDefinitionDiagramLayout.activities;
    var minOffsetTop = 10000;
    activities.forEach(function(item) {
      if (item.y < minOffsetTop) {
        minOffsetTop = item.y;
      }
    });
    minOffsetTop -= 30;
    activities.map(function(item) {
        item.y = item.y - minOffsetTop;
        return item;
    });
		...
		var sequenceFlows = processDefinitionDiagramLayout.sequenceFlows;
    sequenceFlows.map(function(item) {
      item.yPointArray[0] = item.yPointArray[0] - minOffsetTop;
      item.yPointArray[1] = item.yPointArray[1] - minOffsetTop;
      return item;
    });
	...
	setHighLights: function(highLight, processDefinitionId){
		...
		if(highLight instanceof Array){
			highLight.forEach(function(hl){
				var hlnode = ProcessDiagramGenerator.processDiagrams[ProcessDiagramGenerator.options.processDefinitionId].processDefinitionDiagramLayout.highLightInstsMap[hl];

				if(!hlnode.rejected){
					processDiagramCanvas.highLightActivity(hl);
					if(hlnode.isLast){
						processDiagramCanvas.drawLastActivity(hl);
					}
			...
			if(!highLight.rejected){
    			processDiagramCanvas.highLightActivity(highLight);
			}
		}
    ...
    ProcessDiagramGenerator.setHighLights(ProcessDiagramGenerator.options.highlightActivity, ProcessDiagramGenerator.options.processDefinitionId);
	},

  zoomClick: function(e) {
	  var zoomNum  = ProcessDiagramGenerator.options.zoomNum;
    if ((zoomNum >= -3 && zoomNum <= 3) || (zoomNum < -3 && e === '/') || (zoomNum > 3 && e === '*')) {
      var processDiagram = this.getProcessDiagram(ProcessDiagramGenerator.options.processDefinitionId);
      if (processDiagram === undefined) {
        return;
      }

      var processDiagramCanvas = processDiagram.diagramCanvas;
      if (!processDiagramCanvas.g._viewBox) {
        processDiagramCanvas.g.setViewBox(0, 0, parseInt($(".diagram svg").css("width")), parseInt($(".diagram svg").css("height")));
      }
      if ( e == '*') {
        $(".diagram, .diagram svg").css({ width: $(".diagram svg").width() * 0.8, height: $(".diagram svg").height() * 0.8 });
        if (zoomNum > 0) {
          $("#diagramHolder").css({ width: $("#diagramHolder").width() * 0.8, height: $("#diagramHolder").height() * 0.8 });
          $("#diagramHolderBox").scrollTop($("#diagramHolder").height() / 2 - $("#diagramHolderBox").height() / 2);
          $("#diagramHolderBox").scrollLeft($("#diagramHolder").width() / 2 - $("#diagramHolderBox").width() / 2);
        }
        // $("#diagramHolder").css("marginTop", 0);
        // if ($("#diagramHolder").height() > $("#diagramHolderBox").height()) {
        //   $("#diagramHolderBox").scrollTop($("#diagramHolder").height() / 2 - $("#diagramHolderBox").height() / 2);
        //   $("#diagramHolderBox").scrollLeft($("#diagramHolder").width() / 2 - $("#diagramHolderBox").width() / 2);
        // } else {
        //   $("#diagramHolder").css("marginTop", $("#diagramHolderBox").height() / 2 - $("#diagramHolder").height() / 2);
        // }
        ProcessDiagramGenerator.options.zoomNum -= 1;
      } else {
        $(".diagram, .diagram svg").css({ width: $(".diagram svg").width() / 0.8, height: $(".diagram svg").height() / 0.8 });
        if (zoomNum >= 0) {
          $("#diagramHolder").css({ width: $("#diagramHolder").width() / 0.8, height: $("#diagramHolder").height() / 0.8 });
	...
	drawDiagram: function(processInstanceId) {
		// Hide all diagrams
    ...
    if (processInstanceId) {
      this._drawDiagram.apply({processInstanceId: processInstanceId});
    }
	},
	_drawDiagram: function() {
		var processInstanceId = this.processInstanceId;
    ActivitiRest.getProcessDefinitionByInstanceId(processInstanceId, ProcessDiagramGenerator._generateDiagram, ProcessDiagramGenerator._getProcessDefinitionId);
	},

   _getProcessDefinitionId: function (processDefinitionId) {
	...
	_generateDiagram: function() {
		...
		ProcessDiagramGenerator.addBreadCrumbsItem(processDefinitionId, processDefinitionDiagramLayout.processDefinition.name);
	...
	addBreadCrumbsItem: function(processDefinitionId, title){
