...
// Colors
var TASK_COLOR = Color.white; // original: Color.get(255, 255, 204);
var TASK_STROKE_COLOR = '#2B97E0';
/*Color.SlateGrey; */
...
var ACTIVE_COLOR = Color.Firebrick4;
//var SEQUENCEFLOW_COLOR = Color.DimGrey;
var SEQUENCEFLOW_COLOR = Color.get(135,206,235,0.2);

var CATCHING_EVENT_COLOR = Color.black;
/* Color.SlateGrey; */
var START_EVENT_COLOR = Color.get(251, 251, 251);
var START_EVENT_STROKE_COLOR = Color.black;
/* Color.SlateGrey; */
...
// Fonts
var NORMAL_FONT = {font: "10px Arial", opacity: 1, fill: '#93C7EB'};
...
var TASK_FONT = {font: "11px Arial", opacity: 1, fill: '#2B97E0'};
...
var imgArr = {
  "userTask": {
    "normal": "images/deployer/user_normal.png",
    "highLight": "images/deployer/user_active.png",
    "active": "images/deployer/user_active.png"
  }
};
// var USERTASK_IMAGE = "images/deployer/user_normal.png";
...
// var DefaultProcessDiagramCanvas = {
    ...
    init: function (width, height, processDefinitionId) {
        ...
        document.getElementById("diagramHolder").style.minWidth = this.canvasWidth;
        // document.getElementsByTagName("body")[0].style.minWidth = this.canvasWidth;

        this.g = Raphael(this.canvasHolder, this.canvasWidth, this.canvasHeight);
        this.g.clear();
    ...
    setTaskHistory: function(json) {
      ProcessDiagramGenerator.options.taskHistoryList = json;
    },

    getTaskHistory: function() {
        return ProcessDiagramGenerator.options.taskHistoryList;
    },
    ...
    onClick: function (event, instance, element) {
        ...
        if (contextObject.properties.type == 'startEvent' || contextObject.properties.type == 'endEvent') {
        ...
        //TODO: 需要实现这些方法
        if (ProcessDiagramGenerator.options.activeActivity != element.id) {
          this.drawActiveActivity(element.id);
          // console.log(ProcessDiagramGenerator)
        //   if (ProcessDiagramGenerator.options.activeActivity) {
        //     if (ProcessDiagramGenerator.options.activeActivity == ProcessDiagramGenerator.options.highlightActivity) {
        //       this.highLightActivity(ProcessDiagramGenerator.options.activeActivity);
        //     } else {
        //       this.normalActivity(ProcessDiagramGenerator.options.activeActivity);
        //     }
        //   }
        //   ProcessDiagramGenerator.options.activeActivity = element.id;
        ...
        if (ProcessDiagramGenerator.options && ProcessDiagramGenerator.options.on && ProcessDiagramGenerator.options.on.click) {
          ...
          // ProcessDiagramGenerator.options.taskHistoryList = [];
          // ProcessDiagramGenerator.options.on.click.apply(event, args);
          if (ProcessDiagramGenerator.options.taskHistoryList && ProcessDiagramGenerator.options.taskHistoryList.length > 0) {
            args.push(ProcessDiagramGenerator.options.taskHistoryList);
            ProcessDiagramGenerator.options.on.click.apply(event, args);
          } else {
            ActivitiRestDefer.getTaskHistory(args, this.setTaskHistory, ProcessDiagramGenerator.options.on.click);
          }
          var args = [instance, element, contextObject];
    ...
    addHandlers: function (set, x, y, width, height, type) {
        ...
        if(type === 'gateway' && contextObject.properties.name){

            $('#' + contextObject.id).qtip({
                content: function() {
                    // console.log(contextObject.properties)
                    var vars = {'条件网关':contextObject.properties.name};
                    // $.each(contextObject.properties, function(key, value) {
                    //     if(key == "name") {
                    //         Object.assign(vars, {"节点名": value});
                    //     } else if(key == "type") {
                    //         Object.assign(vars, {"任务类型": value});
                    //     } else if(key == 'documentation') {
                    //         Object.assign(vars, {"任务说明": value});
                    //     } else if(key == "isInterrupting") {
                    //         Object.assign(vars, {"是否中断": value ? "是" : "否"});
                    //     } else if (key == "assignee") {
                    //         Object.assign(vars, {"执行人": value});
                    //     } else if (key == "candidateGroups") {
                    //         var valueRes = '(';
                    //         value.forEach(function(item) {
                    //           var valueTemp = '';
                    //           if (item.indexOf('.') >= 0) {
                    //             item.split('.').reduce(function(prev, item1) {
                    //               return prev += item1 + ',';
                    //             }, valueTemp);
                    //             valueTemp = valueTemp.substr(0, valueTemp.length - 1);
                    //           } else {
                    //             valueTemp += item;
                    //           }
                    //           valueRes += valueTemp + '),(';
                    //         });
                    //         Object.assign(vars, {"可指派执行人": valueRes.substr(0, valueRes.length - 2)});
                    //     }
                    // });
                    var tipContent = "<table cellSpacing=0 cellPadding=0 class='need-border'>";
                    $.each(vars, function(varKey, varValue) {
                        if (varValue) {
                            tipContent += "<tr><td class='label' valign='top'>" + varKey + "</td><td style='white-space: nowrap;'>" + varValue + "<td/></tr>";
                        }
                    });
                    tipContent += "</table>";
                    return tipContent;
...
// TODO: generate polylineId or do something!!
    ...
    _drawTask: function (name, x, y, width, height, thickBorder) {
        ...
        shape.attr(attr);
        shape.glow({ width: 4, fill: false, color: '#2b97e0' });
    ...
    drawUserTask: function (name, x, y, width, height) {
        ...
        var img = this.g.image(imgArr.userTask.normal, x + ICON_PADDING, y + ICON_PADDING, ICON_SIZE, ICON_SIZE);
        var contextObject = this.getConextObject();
        if (contextObject) {
          img.id = contextObject.id + 'Image';
        }
    ...
    highLightActivity: function (activityId) {
        ...
        shape.id.toLowerCase() === 'end' && (shape.prev.prev.attr("fill",'#F00'));

        var contextObject = shape.data("contextObject");
        if (contextObject) {
          var img = this.g.getById(activityId + 'Image');
          img && (img.attr('src', imgArr[contextObject.properties.type].highLight));
        ...
        shape.attr("fill", '#FFA94B');
        shape.next.attr('fill', '#FFFFFF');
        // shape.glow({ width: 14, fill: 'red', color: 'red' });
        shape.node.id = activityId;
        var that = this;
        $("#" + activityId).click(function(event) {
          that.onClick(event, that, shape);
        });
    },

    drawLastActivity: function(activityId) {
      var shape = this.g.getById(activityId);
      if(!shape)return;
      shape.glow({ width: 4, fill: 'red', color: 'red' });
      shape.glow({ width: 4, fill: 'red', color: 'red' });
      shape.glow({ width: 4, fill: 'red', color: 'red' });
      shape.glow({ width: 4, fill: 'red', color: 'red' });
    },

    drawActiveActivity: function(activityId) {
      // var shape = this.g.getById(activityId);
      // if(!shape)return;
      // shape.glow({ width: 4, fill: 'red', color: 'red' });
      // shape.glow({ width: 4, fill: 'red', color: 'red' });
      ...
      // else
      //   console.log("--> highLightActivity: ", shape, shape.data("contextObject"));
      // if (activityId == ProcessDiagramGenerator.options.highlightActivity) {
      //   shape.attr("stroke-width", THICK_TASK_BORDER_STROKE);
      //   shape.attr("stroke", "FFA94B");
      // } else {
      //   shape.attr("stroke-width", 0);
      //   // shape.attr("stroke", ACTIVE_COLOR);
      // }
      // shape.attr("fill", '#2B97E0');
      // shape.next.attr('fill', '#FFFFFF');
      // var img = this.g.getById(activityId + 'Image');
      // img.attr('src', imgArr[contextObject.properties.type].active);
    },
    normalActivity: function(activityId) {
      var shape = this.g.getById(activityId);
      if (!shape) {
        console.error("Activity " + activityId + " not found");
        return;
      ...
      shape.attr("stroke-width", NORMAL_STROKE);
      shape.attr("stroke", TASK_STROKE_COLOR);
      shape.attr("fill", TASK_COLOR);
      shape.next.attr('fill', '#2B97E0');
      var img = this.g.getById(activityId + 'Image');
      img.attr('src', imgArr[contextObject.properties.type].normal);
    },
    ...
    drawLabel: function (text, x, y, width, height, labelAttrs) {
        var debug = false;
